...
//TODO: MOVE OPTIONS TO LOCALSTORAGE
                $(".setlist-header").remove();
                optionsHeader = $('<h2 class="text-center setlist-header">Options</h2>');
...
function displayLyrics(song) {
   ...
   lyricsApi = lyricApi + "v1/" + (song.artist).replace(/' '/g, '%20') + "/" + (song.title).replace(/' '/g, '%20');
    ...
    $.getJSON(lyricsApi).fail(function() {
        errorDisplay = "<h2 class='setlist-header'>Sorry, but we're having trouble getting the information for that song. Please try another. </h2>"
        results.append(errorDisplay);
        return;
    }).then(function(data) {
    //Add button to save to setlist
   var lyricDisplay = '<h2 class="setlist-header">' + song.display + '</h2>';
...
function displayMeta(data) {
    ...
    fetch(apiURL).then(result => result.json()).then(data => {
        ...
        noSongData = "<h2 class='setlist-header'>There is no song data available for this song. </h2>"
...
//TODO: GETLOCALSTORAGE FUNCTION
    ...
    getLocalStorage = function() {
        ...
        $("#metadata").remove();
        $("#save-setlist").remove();
        setlistHeader = $("<h2 class='setlist-header'>Setlist </h2>") 
